...
package fr.ign.cogit.core;

import fr.ign.cogit.graphics.Interface;
...
import fr.ign.cogit.network.Graph;
import fr.ign.cogit.network.Network;
import fr.ign.cogit.util.Loaders;
import fr.ign.cogit.util.Parameters;
import fr.ign.cogit.util.Project;
import fr.ign.cogit.util.Tools;
...
public class MapMatching {
	...
	// -------------------------------------------------------------------
	// Module for setting map-matching parameters
	// -------------------------------------------------------------------
				...
				for (int k=0; k<CANDIDATES_C.get(i).size(); k++){
					...
					if (CANDIDATES_DT.get(i) == 0){
						...
						if (MapMatching.gui_mode){
							...
							Tools.println("Error: two consecutive points with same timestamp"+track.getPath() + "(" + i + "," + k + ")"
									+ track.getX().get(i) + "--" + track.getY().get(i));
							System.exit(12);
	...
	private static File printReport(){
		...
		try {
			...
			double mad = -1;
			if (MapMatching.getDisplacementX().size() > 0) {
				mad = Tools.round(Tools.mad(MapMatching.getDisplacementX(), MapMatching.getDisplacementY()), 2);
			} else {
				System.out.println("-----------");
			}
